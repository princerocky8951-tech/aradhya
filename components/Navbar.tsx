          ...
          <div className="hidden md:block">
            <div className="ml-10 flex items-baseline space-x-8">
              ...
              {user ? (
                ...
                <Link to="/login" className="hidden ml-4 px-6 py-2 bg-crimson-900/20 border border-crimson-900/50 text-crimson-500 hover:bg-crimson-900/40 text-xs font-bold uppercase tracking-widest rounded transition-all">
                  Login
                </Link>
              )}
            </div>
          ...
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            {links.map((link) => (
             ...
             {user ? (
               ...
               <Link to="/login" onClick={() => setIsOpen(false)} className="hidden text-crimson-500 block px-3 py-2 text-base font-medium uppercase tracking-wider">Login</Link>
             )}
          </div>
        </div>
      )}
